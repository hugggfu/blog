  ...
  <div style="color: #666">
    <div style="margin: 20px 0; ">
      ...
      <div style="font-size: 14px; margin-top: 10px">
        ...
        <i class="el-icon-view" ></i> <span>{{article.viewCount}} 次浏览 •</span>
        <i class="el-icon-chat-dot-round" ></i> <span>{{article.commentCount}} 评论 •</span>
      </div>
    </div>
   ...
   <div style="margin: 20px 0">
    ...
    <div style="margin: 30px 0">
      <div style="margin: 10px 0">
        <div style="border-bottom: 1px solid orangered; padding: 10px 0; font-size: 20px">评论  <i class="el-icon-chat-dot-round" ></i>{{article.commentCount}}</div>
...
export default {
  ...
  methods: {
    load() {
      this.request.get("/front/article/" + this.id).then(res => {
        this.article = res.data
      })
      this.request.put("/front/article/updateViewCount/" + this.id)
    },
    loadComment() {
      this.request.get("/front/comment/tree/" + this.id).then(res => {
    ...
    save() {
      ...
      this.request.post("/front/comment", this.commentForm).then(res => {
        if (res.code === 200) {
    ...
    del(id) {
      this.request.delete("/front/comment/" + id).then(res => {
        if (res.code === 200) {
